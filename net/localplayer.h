...
typedef struct _ONFOOT_SYNC_DATA
{
	...
	uint16_t wAnimation;			// +64
	uint16_t wAnimationFlags;		// +66
...
public:
	...
	void SendGiveDamageEvent(uint16_t wPlayerID, float damage_amount, uint32_t weapon_id, uint32_t bodypart);
	void SendTakeDamageEvent(uint16_t wPlayerID, float damage_amount, uint32_t weapon_id, uint32_t bodypart);
	void SendGiveDamageActorEvent(uint16_t wPlayerID, float damage_amount, uint32_t weapon_id, uint32_t bodypart);
