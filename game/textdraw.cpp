...
void CTextDraw::DrawTextured()
{
	...
	// 	pChatWindow->AddDebugMessage("Warning: Invalid texture expected");
	// 	Log("Warning: Invalid texture expected");
	// 	return;
	// }
...
void CTextDraw::LoadTexture()
{
	...
	if (strlen(m_szText) < 64 && strchr(m_szText, '\\') == nullptr && strchr(m_szText, '/') == nullptr)
	{
		...
		if (m_TextDrawData.iTextureSlot != -1) {
			TextDrawTexture[m_TextDrawData.iTextureSlot] = ::GetTexture(texturename);
		}
	}
