...
void CEntity::Add()
{
	if(!m_pEntity || IsGameEntityArePlaceable(m_pEntity) || m_pEntity->dwUnkModelRel)
		return;
...
{
	if(!m_pEntity || IsGameEntityArePlaceable(m_pEntity) || !m_pEntity->m_pRwObject || !m_pEntity->mat)
		return;
...
void CEntity::SetAndUpdateMatrix(MATRIX4X4 Matrix)
{
	...
	SetMatrix(Matrix);
	TeleportTo(Matrix.pos.X, Matrix.pos.Y, Matrix.pos.Z);
}

void CEntity::SetCollision(bool bSt)
{
	if(!m_pEntity || IsGameEntityArePlaceable(m_pEntity))
		return;

	m_pEntity->nEntityFlags.m_bUsesCollision = bSt ? 1 : 0;
}
...
void CEntity::SetGravityProcessing(bool bProcess)
{
	...
	/*if(bProcess)
		m_pEntity->m_nEntityFlags &= 0x7FFFFFFD;
	else
		m_pEntity->m_nEntityFlags |= 0x80000002;*/
}
