			...
			if(pGame)
			{
				for(PLAYERID playerId = 0; playerId < MAX_PLAYERS; playerId++)
				{
					if(pPlayerPool->GetSlotState(playerId) == true)
					{
						...
						if(pPlayer && pPlayer->IsActive() && pPlayer->m_bShowNameTag && !pPlayer->IsNPC())
						{
							CPlayerPed* pPlayerPed = pPlayer->GetPlayerPed();
							if(pPlayerPed && pPlayerPed->GetDistanceFromCamera() <= pNetGame->m_fNameTagDrawDistance)
							{
								/*if(pPlayer->GetState() == PLAYER_STATE_DRIVER && pPlayer->m_pCurrentVehicle && pPlayer->m_pCurrentVehicle->IsRCVehicle())
								{
